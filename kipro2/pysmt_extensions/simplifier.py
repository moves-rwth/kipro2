...
class Simplifier(pysmt.walkers.DagWalker):
    ...
    def walk_plus(self, formula, args, **kwargs):
        ...
        # Flatten Plus(Plus(a,b),c) to Plus(a,b,c)
        new_args = []
        for formula in args:
            if formula._content.node_type != 13:
                new_args.append(formula)
            else:
                for form in formula._content.args:
                    new_args.append(form)

        args = new_args

        ns = [x for x in args if not x.is_constant()]

        #print([x.serialize() for x in ns])
